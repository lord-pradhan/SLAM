...
%     fusion_map.pointcloud.Location
    % transform to camera frame
%     points_camera = sum(T_inv_mat .* permute(points_ground, [2 1 3]) , 2);   
%     points_camera = cat(1, reshape( permute(pc_camera.Location,[3 2 1]), 3,1, [] ), ones(1,1, m) ); 
...
%     points_camera = points_camera(1:3,:,:); %todo: z>0
%     points_ground = points_ground(1:3,:,:);
...
%     ccount_fov = ccount_camera(:,:,fov_idxs);
%     times_fov = times_camera(:,:,fov_idxs);
